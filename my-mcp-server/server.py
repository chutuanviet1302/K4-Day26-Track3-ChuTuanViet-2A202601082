...
Triển khai đầy đủ 3 bài tập (Bài 1, Bài 2, Bài 3) và 7 Bước Thực hành:
- Bước 1-3: MCP Server với tools thực tế (get_order, get_order_v2, search_orders, search_logs).
- Bước 4-5: Hỗ trợ Authentication qua HTTP (Streamable HTTP với Bearer Token Verifier).
- Bước 6: Hỗ trợ Versioning (v1, v2, resource server://info).
"""
...
# --- Danh sách Token hợp lệ (Bước 5: Authentication) ---
...
class StaticTokenVerifier(TokenVerifier):
    """Xác minh Bearer Token tĩnh cho Streamable HTTP transport (Bước 5)."""
...
def search_logs(keyword: str = "", level: str = "ERROR", limit: int = 10) -> str:
    ...
    for log in SYSTEM_LOGS:
        match_level = level.upper() == "ALL" or log["level"].upper() == log["level"].upper()
        match_kw = not keyword or (
...
# ── BƯỚC 6: RESOURCE SERVER METADATA ────────────────────────────────
